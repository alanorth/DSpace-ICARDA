...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template name="authorityConfidenceIcon">
      ...
      <span i18n:attr="title">
        ...
        <xsl:attribute name="class">
          <xsl:text>ds-authority-confidence glyphicon btn-xs </xsl:text>
          <xsl:choose>
            <xsl:when test="string-length($lcConfidence) > 0">
              <xsl:value-of select="concat('cf-',$lcConfidence,' ')"/>
                <xsl:choose>
                    <xsl:when test="$lcConfidence = 'accepted'">
                        <xsl:text> glyphicon-thumbs-up </xsl:text>
                    </xsl:when>
                    <xsl:when test="$lcConfidence = 'unset'">
                        <xsl:text> glyphicon-remove </xsl:text>
                    </xsl:when>
                    <xsl:when test="$lcConfidence = 'novalue'">
                        <xsl:text> glyphicon-ban-circle </xsl:text>
                    </xsl:when>
                    <xsl:when test="$lcConfidence = 'failed'">
                        <xsl:text> glyphicon-warning-sign </xsl:text>
                    </xsl:when>
                    <xsl:when test="$lcConfidence = 'notfound'">
                        <xsl:text> glyphicon-thumbs-down </xsl:text>
                    </xsl:when>
                    <xsl:when test="$lcConfidence = 'ambiguous'">
                        <xsl:text> glyphicon-question-sign </xsl:text>
                    </xsl:when>
                    <xsl:when test="$lcConfidence = 'uncertain'">
                        <xsl:text> glyphicon-cog </xsl:text>
                    </xsl:when>
                </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
              <xsl:text>cf-blank hidden</xsl:text>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:attribute>
    ...
    <xsl:template name="addAuthorityAutocompleteWidgets">
      ...
      <span style="display:none;">
        ...
        <img alt="xmlui.mirage2.choice-authority-control.loading" i18n:attr="alt">
      ...
      <xsl:param name="isClosed" select="'false'"/>
          ...
          <script type="text/javascript">
              if (window.runAfterJSImports == undefined){
                  ...
                  window.runAfterJSImports = new FnArray();
                  }
          </script>
        <script type="text/javascript">
    ...
    <xsl:template match="dri:item[@id='aspect.general.ChoiceLookupTransformer.item.select']/dri:figure">
      <img id="lookup_indicator_id" style="display:none;" alt="xmlui.mirage2.choice-authority-control.loading" i18n:attr="alt">
        <xsl:attribute name="src">
    ...
    <xsl:template match="dri:field[@id='aspect.general.ChoiceLookupTransformer.field.cancel']"></xsl:template>

    <xsl:template match="dri:div[@id='aspect.general.ChoiceLookupTransformer.div.lookup']/dri:head"></xsl:template>
    <xsl:template match="dri:list[@id='aspect.general.ChoiceLookupTransformer.list.choicesList']/dri:head">
